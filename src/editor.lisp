...
(defun handle-key (buf key &optional (session *current-session*))
  ...
  (cond
    ...
    ((eql key :transpose)
     (when (buffer-transpose-chars buf)
       :redraw))
    ;; Ctrl+O - open line (insert newline, cursor stays)
    ((eql key :open-line)
     (buffer-open-line buf)
     :redraw)
    ;; Tab - completion
    ((eql key :tab)
    ...
    ((and (consp key) (eql (first key) :alt) (char-equal (rest key) #\d))
     (reset-prefix-search session)
     (when (buffer-kill-word-forward buf)
       :redraw))
    ;; Alt+Backspace - kill word backward
    ((and (consp key) (eql (first key) :alt) (eql (rest key) #\Rubout))
     (reset-prefix-search session)
     (when (buffer-kill-word-backward buf)
       :redraw))
